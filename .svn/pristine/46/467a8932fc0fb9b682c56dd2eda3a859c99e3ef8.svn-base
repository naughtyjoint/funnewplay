...
if(isset($_GET["carttaction"]) && ($_GET["carttaction"]=="update")){
	if(isset($_GET["updateid"])){
		...
		for($j=0;$j<$i;$j++){
			$cart_rain->edit_item($_GET['updateid'][$j],$_GET['qty'][$j]);
		}
	}
...
if(isset($_GET["carttaction1"]) && ($_GET["carttaction1"]=="update1")){
	if(isset($_GET["updateid1"])){
		...
		for($j=0;$j<$i;$j++){
			$cart_rain->edit_st($_GET['updateid1'][$j],$_GET['st'][$j]);
		}
	}
...
if(isset($_GET["carttaction2"]) && ($_GET["carttaction2"]=="update2")){
	if(isset($_GET["updateid2"])){
		...
		for($j=0;$j<$i;$j++){
			$cart_rain->edit_ot($_GET['updateid2'][$j],$_GET['ot'][$j]);
		}
	}
...
if(isset($_GET["carttaction3"]) && ($_GET["carttaction3"]=="update3")){
	if(isset($_GET["updateid3"])){
		...
		for($j=0;$j<$i;$j++){
			$cart_rain->edit_date($_GET['updateid3'][$j],$_GET['date'][$j]);
		}
	}
  ...
  <tr>
    <td class="tdbline"><table width="100%" border="0" cellspacing="0" cellpadding="10">
        <tr valign="top">
          ...
          <?php		  
		  	foreach($cart_rain->get_contents() as $item) {
		  ?>              
              <tr>
          ...
          <?php }?>
            ...
            <input name="carttaction1" type="hidden" id="carttaction1" value="update1">       
			  ...
			  <?php }?>
              <input type="button" name="backbtn" id="button4" value="回上一頁" onClick="location.href='falistrain.php?pid=<?php echo $row_RecBill["Pla_id"];?>'">
              </p>
          </form>
          </div>          
            <?php }else{ ?>
            ...
            <p align="center">
             <input type="button" name="backbtn" id="button4" value="回上一頁" onClick="location.href='falistrain.php?pid=<?php echo $row_RecBill["Pla_id"];?>'"></p></form> <?php } ?> </td>
        </tr>
    </table></td>
  </tr>
  <tr>
