...
//檢查是否為會員
if(isset($_POST["m_mail"])){	
	//找尋該會員資料
	$query_RecFindUser = "SELECT * FROM `member` WHERE `Mem_email`='".$_POST["m_mail"]."'";
	$RecFindUser = mysql_query($query_RecFindUser);	
	if (mysql_num_rows($RecFindUser)==0){
		header("Location: admin_passmail.php?errMsg=1&usermail=".$_POST["m_mail"]);
	}else{	
	//取出帳號密碼的值
		...
		$query_update = "UPDATE `member` SET `Mem_pass`='".md5($newpasswd)."' WHERE `Mem_email`='".$usermail."'";
...
<table width="780" border="0" align="center" cellpadding="4" cellspacing="0">
  <tr>
    <td class="tdbline"> <a href="../phpmember/index.php"><img src="images/logo1.jpg" alt="會員系統" width="301" height="168"></td>
  </tr>
  <tr>
...
<div class="regbox">
          ...
          <div class="errDiv">此帳號「 <strong><?php echo $_GET["usermail"];?></strong> 
            」
            沒有人使用！</div>
          <?php }?>
          <p class="heading">忘記密碼？</p>
          <form name="form1" method="post" action="">
            <p>請輸入您申請的信箱帳號，系統將自動產生一個十位數的密碼寄到您註冊的信箱。</p>
            <p><strong>帳號</strong>：
              <br>
              <input name="m_mail" type="text" class="logintextbox" id="m_mail">
            </p>
            <p align="center">
